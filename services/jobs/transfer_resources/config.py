...
import re
from argparse import ArgumentParser, Action as argparseAction, Namespace
...
def create_parse_choice_with_both(transform_function: callable) -> callable:
    """
    Creates a `ParseChoiceWithBoth` action with a transform function.
    """
...
def parse_service_args(parser: ArgumentParser) -> None:
    ...
    parser.add_argument(
        "-hn",
        "--handler-name",
        default="transfer-resources-handler",
        type=str,
        help=(
...
def get_application_config() -> dict:
    ...
    """
    parser = ArgumentParser(
        description="CLI for transfer resources policies job"
    )
